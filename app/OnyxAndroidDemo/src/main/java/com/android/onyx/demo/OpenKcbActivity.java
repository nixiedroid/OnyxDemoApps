...
public class OpenKcbActivity extends AppCompatActivity {
    ...
    private TabIntentData setNoteJumpPath(TabIntentData data) {
        String jumpPath = "";
        int id = binding.rgNote.getCheckedRadioButtonId();
        if (id == R.id.rb_search) jumpPath = NoteRouter.SEARCH.toString();
        else if (id == R.id.rb_backup)  jumpPath = NoteRouter.BACKUP.toString();
        else if (id == R.id.rb_common_setting) jumpPath = NoteRouter.COMMON_SETTING.toString();
        else if (id == R.id.rb_ai_setting) jumpPath = NoteRouter.AI_SETTING.toString();
